...
export default function TagsPage() {
  ...
  async function onSubmit(data: FormData) {
    try {
      ...
      if (!res.ok) throw new Error(json.error || 'Erro ao criar tag')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Crie e gerencie tags usadas no portfólio</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Adicione uma nova tag</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <Label htmlFor="name">Nome</Label>
              <Input id="name" placeholder="Ex: Drone" {...register('name')} disabled={isSubmitting} />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Excluir tags que não usa mais</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium flex items-center gap-2"><Tag className="h-4 w-4" /> {item.name}</TableCell>
                    <TableCell className="text-muted-foreground">{item.slug}</TableCell>
                    <TableCell className="text-right">
                      <Button variant="destructive" size="sm" onClick={() => setPendingDeleteId(item.id)} disabled={deletingId === item.id}>
